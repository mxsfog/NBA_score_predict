...
class ScorePredictionTransformer(nn.Module):
    def __init__(self, input_size=8, d_model=64, num_heads=4, num_layers=3, dropout=0.15):
    ...
    def forward(self, x):
        # x имеет размерность (20, 8)
        x = x.unsqueeze(1)  # Добавляем размерность для batch: (20, 1, 8)

        # Применяем embedding ко всей последовательности
        x = self.embedding(x)  # (20, 1, d_model)
        x = self.pos_encoder(x)

        # Применяем Multi-Head Attention ко всей последовательности
        ...
        output = x.mean(dim=0)  # (1, d_model)

        # Предсказываем счет
        scores = self.fc(output)  # (1, 2)
        return scores.squeeze(0)  # Возвращаем тензор размера (2)